...
class Phone {
    ...
    static async updatePhoneById(phoneId, phone) {
        const { rows } = await pool.query(
            'UPDATE phones SET name=$1, brand=$2, cost=$3 WHERE id=$4 RETURNING *', [phone.name, phone.brand, phone.cost, phoneId]
        );
    ...
    static async deleteById(phoneId) {
        const { rows } = await pool.query(
            'DELETE FROM phones WHERE id=$1 RETURNING *', [phoneId]
        );
